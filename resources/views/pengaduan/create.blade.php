...
<form method="POST" enctype="multipart/form-data" action="{{route('pengaduan.store')}}" class="flex items-center flex-col max-w-96 mx-auto p-4 my-10 bg-green-500 rounded-lg shadow-md">
        @csrf
        <div class="w-full mb-4">
            <label for="alamat" class="block text-white font-bold mb-2">alamat</label>
            <textarea name="alamat" id="alamat" placeholder="Tuliskan alamat lengkap Anda di sini..." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-400" required>
          ...
          <div class="w-full mb-4">
            ...
            <input type="text" name="isi_laporan" id="isi_laporan" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-400" required>
        </div>
        ...
        <button type="submit"
        class="bg-white text-green-600 font-bold mt-10 px-4 py-2 rounded hover:bg-gray-100">
        Kirim Pengaduan
    </button>
    </form>
